...
async function scrapeMarketplace(releaseId) {
  try {
    const url = `https://www.discogs.com/sell/release/${releaseId}?sort=price%2Casc`;
...
app.get('/analyze', async (req, res) => {
  ...
  try {
    ...
    if (!wantlistRes.ok) {
      if (wantlistRes.status === 404) return res.status(404).json({
cat > server.js << 'EOF'
import express from 'express';
import cors from 'cors';
import * as cheerio from 'cheerio';
...
app.get('/analyze', async (req, res) => {
  ...
  try {
    ...
    const { wants = [] } = await wantlistRes.json();
